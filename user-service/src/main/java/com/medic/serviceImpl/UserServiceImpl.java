...
public class UserServiceImpl implements UserService {
    ...
    public Boolean signup(String email, String name, int age, String gender) {
        ...
        if (user == null) {
            UserEntity userinfo = UserEntity.builder().email(email).name(name).age(age).gender(gender).build();
...
//    }*/
    ...
    @Transactional
    public List<LikeEntity> getUserLike(int userId) {
        return likeRepository.findAllByUserId(userId);
    }

    @Transactional
    public List<Integer> likeListOfSupplement(int supplementId) {
        List<LikeEntity> list = likeRepository.findAllBySupplementId(supplementId);
    ...
    @Transactional
    public void updateName(UserDTO.ReqUpdateName updateNameReq) {
        userRepository.updateName(updateNameReq.getUserId(), updateNameReq.getName());
    }

    @Transactional
    public String[] userFirstSurvey(UserDTO.ReqFirstSurvey userFirstSurveyReq) {
...
//		}
        ...
        CommonQuestionEntity cq = CommonQuestionEntity.builder()
                ...
                .outdoorActivity(userFirstSurveyReq.getOutdoor_activity())
                .balancedMeal(userFirstSurveyReq.isBalanced_meal())
                .lack(userFirstSurveyReq.getLack())
                .isOkBigPill(userFirstSurveyReq.is_ok_big_pill())
                ...
                .kidneyDisease(userFirstSurveyReq.getDisease().contains("신장 질환"))
                ...
                .preferredBrand(userFirstSurveyReq.getPreferred_brand())
    ...
    public void insertDetail(HealthDTO.ReqDetail detailHealthReq) {
        CommonQuestionEntity cq = CommonQuestionEntity.builder().allergy(null).balancedMeal(false).constipation(false)
                .diarrhea(false).heartburn(false).isOkBigPill(detailHealthReq.getPillSize()).kidneyDisease(false)
                .lack(null).preferredBrand(detailHealthReq.getBrand()).pregnant(false).problem(null)
                .outdoorActivity(0).smoking(false).userId(detailHealthReq.getUserId()).build();
        commonQuestionRepository.save(cq);
    }
    ...
    @Transactional
    public List<UserDTO.CompareUser> calcSimilarity(CommonQuestionEntity cq, int age, String gender, int uuId) {
        // 나이와 성별로 1차 필터링한 유저 리스트 받아오기
        List<UserEntity> userList = userRepository.findByAgeAndGender(age, gender, uuId);
        ...
        Integer outdoor_activity = cq.getOutdoorActivity();
        Boolean balanced_meal = cq.getBalancedMeal();
        String lack = cq.getLack();
        Boolean is_ok_big_pill = cq.getIsOkBigPill();
        ...
        Boolean kidney_disease = cq.getKidneyDisease();
        ...
        String preferred_brand = cq.getPreferredBrand();
        String problem = cq.getProblem();

        List<UserDTO.CompareUser> comparedUser = new ArrayList<>();

        // 리스트에 있는 유저별로 cq를 받아오고 각각 비교
        for (UserEntity u : userList) {
            int cnt = 0;
            int userId = u.getUserId();
            CommonQuestionEntity ucq = commonQuestionRepository.findOneByUserId(userId);
            ...
            if (ucq.getAllergy().contains(allergy) || allergy.contains(ucq.getAllergy()))
                cnt++;
            if (Objects.equals(ucq.getOutdoorActivity(), outdoor_activity))
                cnt++;
            if (ucq.getBalancedMeal() == balanced_meal)
                cnt++;
            if (ucq.getLack().contains(lack) || lack.contains(ucq.getLack()))
                cnt++;
            if (ucq.getIsOkBigPill() == is_ok_big_pill)
                cnt++;
            ...
            if (ucq.getThyroidDisease() == thyroidDisease)
                cnt++;
            if (ucq.getKidneyDisease() == kidney_disease)
                cnt++;
            ...
            if (ucq.getPhysicalActivity() == physicalActivity)
                cnt++;
            if (ucq.getPreferredBrand().contains(preferred_brand)
                    || preferred_brand.contains(ucq.getPreferredBrand()))
                cnt++;
            if (ucq.getProblem().contains(problem) || problem.contains(ucq.getProblem()))
                cnt++;

            UserDTO.CompareUser cu = new UserDTO.CompareUser(userId, cnt);
            comparedUser.add(cu);
        }
